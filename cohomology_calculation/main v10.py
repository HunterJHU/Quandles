...
def computeCoeff(triple, matrix):
	...
	# 	if current_eq[i] != 0:
	# 		equation.append("%d(%d,%d)" % (current_eq[i] , (i / n), (i % n)))
	# print equation, '\n'
	return current_eq
...
for i in range(rows):
	...
	for j in range(i+1, rows):
		if (store==relations[j]).all():
			print i,j
			relations = delete(relations, j, axis=0)
			rows -= 1
			print relations
	print i
...
print Poly(function.subs(solve(cob_gens)), variables)
print

final_list = []
for stuff in Poly(function.subs(solve(cob_gens)), variables).coeffs():
	print stuff.expand(modulus=mod_num)
